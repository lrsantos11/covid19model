...
df_SC <- read.csv(paste0(path,"/sc-deaths.csv"), stringsAsFactors = FALSE)
df_SC$DateRep <- as.POSIXct(as.character(df_SC$data))
colnames(df_SC) <- c("fullregion","region","date","Cases","cumulative_cases","Deaths","cumulative_deaths","DateRep")

df_SC <- df_SC[c("DateRep","region","Cases","Deaths")]

regions <- unique(df_SC$region)
days<-length(seq(as.Date('2020/01/01'),as.Date(df_SC$DateRep[1]-1),"days"))
zeroes<-data.frame(DateRep=rep(seq(as.Date('2020/01/01'),as.Date(df_SC$DateRep[1]-1),"days"),length(regions)),                   region=regions[rep(seq_len(length(regions)), each = days)],                  Cases=0,Deaths=0)
zeroes$DateRep <- as.POSIXct(zeroes$DateRep)
df_SC=bind_rows(zeroes,df_SC)
...
write_csv(df,"Brazil/data/df-sc.csv")
...
mobility.aux2 = mobility.aux2[which(mobility.aux2$iso_3166_2_code=="BR-SC"),]
...
colnames(mobility)[grep("iso_3166_2_code",colnames(mobility))] = "region"
...
write_csv(mobility,paste0(path,"/mobility-processed-sc.csv"))
...
process_data <- function(countries,N2,df){
  ...
  pad_serial.interval <- data.frame(    "X"=(length(serial.interval$fit)+1):200,    "fit"=rep(1e-17, 200))
  ...
  for(Country in countries) {
    ...
    aux.int = interventions[interventions$region=="SC",]
    ...
    covariates2[(N+1):(N+forecast),] <- cbind(mat.forecast$grocery_pharmacy,mat.forecast$parks,mat.forecast$residential,mat.forecast$retail_recreation, mat.forecast$transitstations,mat.forecast$workplace)
